...
if (!target || target.startsWith('--')) { console.error('Usage: npm run verify:emotion -- --target <clean-baseline-repo>'); process.exit(2); }
...
execFileSync(process.execPath, [path.join(root, 'scripts/apply-module.mjs'), 'emotion-safety', '--target', resolved, '--write'], { stdio: 'inherit' });
const testFiles = [
  'utils/emotionBuff.test.ts',
  'utils/emotionApply.test.ts',
  'utils/contextVolatileSplit.test.ts',
  'utils/emotionAmsgRace.test.ts',
];
