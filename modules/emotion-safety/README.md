...
此模块须应用到 `core-integration` 的精确结果 tree 上。它只保留经单行化、控制字符过滤和限长处理的 `activeBuffs` 抽象标签与强度，阻止旧事件或临时场景进入跨回合注入，并在 AMSG 回合收尾从 DB 重读角色以避免旧闭包覆盖新情绪状态；`emotionAmsgRace.test.ts` 是该静态接线的回归守卫，不是运行时竞态测试。
